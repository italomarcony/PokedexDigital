...
@Component({
  ...
  template: `
    ...
    <h2>Equipe de Batalha</h2>
    <p style="color:#666; margin-bottom:16px;">Máximo de 6 Pokémon na equipe | {{ items().length }}/6</p>
...
export class TeamComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    // Recarregar quando a navegação retornar para /team
  ...
  reload() {
    this.userService.getTeam().subscribe({
      ...
      error: () => this.error.set('Falha ao carregar equipe (necessário login).')
    });
  }
  ...
  remove(pokemon: any) {
    this.userService.removeFromTeam(pokemon.IDPokemonUsuario).subscribe({
      next: () => {
        this.items.set(this.items().filter(x => x.IDPokemonUsuario !== pokemon.IDPokemonUsuario));
        this.showToast(`${pokemon.Nome} removido da equipe!`, 'success');
      },
      error: () => {
        this.showToast('Falha ao remover da equipe', 'error');
      }
    });
  }
}